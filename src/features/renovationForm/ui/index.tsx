...
type Props = {
    ...
    imgs: ImgType[];
    setRenovationType: (type: string) => void;
...
function RenovationForm({
    ...
    useEffect(() => {
        if (!isPending && data) {
            ...
            if (data?.data?.repair)
                setRenovationType(
                    getRenovationTypeFromResponse(data?.data?.repair)
                );
        ...
        <>
            <div className={`${className} flex flex-col`}>
                <div className="gap-5 flex flex-col">
                    <h2 className="prose-lg  font-semibold">
            ...
            {isError ? (
                <AlertComponent
                    ...
                    active={alertIsShowing}
                >
                    <>
                        <h2 className="prose-md font-bold">
                            Не удалось получить тип ремонта!
                        </h2>
                        <p className="prose-sm">{`Попробуйте другие фото или зайдите позже`}</p>
                    </>
                </AlertComponent>
            ) : (
                <AlertComponent
                ...
                >
                    <>
                        <h2 className="prose-md font-bold">
                            У вашей квартиры тип -{" "}
                            {currentRenovationType.toLowerCase()}
                        </h2>
